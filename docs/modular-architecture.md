...
4. Continue decomposing expedition watch-form policy and mutation submission only where the lifecycle boundary is clear. Read-only expedition status/history/presentation projection is already isolated in `expedition-presentation.ts`, while Source Maps owns dedicated registration and Wonderdraft controllers.
