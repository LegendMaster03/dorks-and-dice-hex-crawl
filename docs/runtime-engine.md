...
`AutomaticRoll` is emitted only when the server-side procedure-resolution helper actually generates the corresponding resolved value. Ordinary manual selectors still do not expose `AutomaticRoll`; manual DM entry can record `ProcedureDefault`, `ManualRoll`, `ExternalSystem`, or `DmOverride`. `POST /api/expeditions/{id}/resolution-helper` reads the persisted procedure snapshot and current session state, combines them with explicit DM-confirmed situational inputs, and returns resolved travel/navigation/encounter drafts with roll traces and provenance. Consequential generation is an **audit-only session mutation**: every generated attempt is immediately appended as `ProcedureResolutionHelperGenerated` history and increments the aggregate version before another generation or application can occur. The same transaction persists a structured `GeneratedProcedureResolution` containing a server-generated ID, session ID, generation version, audit sequence, watch number, roll trace, resolved component values, and lifecycle status. It does not advance time, movement, navigation, encounter state, knowledge, or any other mechanical runtime state. The existing `/advance` operation remains the only full-watch application boundary.

`/advance` does not trust client provenance text. A component submitted as `AutomaticRoll` must include the server-generated resolution ID, and the application verifies that the persisted attempt belongs to the same session, is still `Available`, was generated for the current aggregate version and watch context, and exactly matches the submitted generated travel/navigation/encounter values. A reroll marks the prior available attempt `Superseded`; successful application marks the selected attempt `Consumed` and records `ProcedureResolutionHelperConsumed`. Manual or tampered values therefore can not acquire `AutomaticRoll` provenance by supplying a forged note or stale ID. Focused assistant endpoints and the legacy manual advance path also reject client-supplied `AutomaticRoll`; only the verified helper-generation/application path may establish that provenance.
