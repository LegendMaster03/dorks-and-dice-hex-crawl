...
`ExpeditionWorkbenchService` is the full-watch application orchestration layer over the spatial engine. It starts expeditions from procedure/presentation presets, persists customized procedure snapshots, determines whether the configured encounter cadence is still due for the current watch/day, preserves legacy `Custom` cadence behavior, builds independent resolved-input provenance, calls `CrawlRuntimeEngine`, composes world/knowledge projection, applies presentation projection, and saves with optimistic concurrency. A focused Encounter Cadence resolution is authoritative for that same upcoming watch/day, so reopening the full workbench does not create a second encounter check.
